...
import type { DeleteSellerListingDto } from './seller-listing.types.js';

@injectable()
export class DeleteSellerListingUseCase {
  constructor(
    @inject(TOKENS.AdminSellerRepository) private repo: IAdminSellerRepository,
  ) {}

  async execute(dto: DeleteSellerListingDto): Promise<void> {
    if (!dto.listing_id) throw new Error('listing_id is required');
    return this.repo.deleteSellerListing(dto);
  }
}
