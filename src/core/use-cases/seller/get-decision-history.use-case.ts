...
import type { GetDecisionHistoryDto, GetDecisionHistoryResult } from './seller-pricing.types.js';
...
export class GetDecisionHistoryUseCase {
  ...
  async execute(dto: GetDecisionHistoryDto): Promise<GetDecisionHistoryResult> {
    if (!dto.listing_id) throw new Error('listing_id is required');
    return this.repo.getDecisionHistory(dto);
  }
}
